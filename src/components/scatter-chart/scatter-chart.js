...
class ScatterCharts {
  constructor(data) {
    ...
    this.container = document.querySelector(
      ".dv-scatterplot-container-charts--chart"
    );
    ...
    document.querySelector(
      ".dv-scatterplot-container-charts--description"
    ).innerHTML = `The scatterplot shows a relationship between average module attendance and average module marks for courses based in <span class="title-${this.campusName.replace(
      " ",
      ""
    )}" id="campusName">${this.campusName}</span>`;
    document.querySelector(
      ".dv-scatterplot-container-charts--title"
    ).innerHTML = "Attendance and Performance";
    document.querySelector(
      ".dv-scatterplot-container-charts--description-two"
    ).innerHTML = `Based on the individual <span class="dot-scatter dot-${this.campusName.replace(
      " ",
      ""
    )}"></span> data of students, we can see that there is a stronger positive correlation for courses held at Cavendish campus compared to those in the Harrow campus.`;
  ...
  drawScatterplot() {
    ...
    this.svg
      .append("g")
      .attr("class", `container--${this.campusName.replace(" ", "")}`);
  ...
  formatYAxis() {
    d3.select(document.querySelector(".y-axis")).call(
  ...
  addEventListener() {
    document.querySelector("#campus-select").addEventListener("change", (i) => {
      ...
      d3.select(
        `.dv-scatterplot-container-charts--chart .container--${this.campusName.replace(
          " ",
          ""
        )}`
      ).remove();
  ...
  resize() {
    const chart = d3.select(
      document.querySelector(".dv-scatterplot-container-charts--chart svg")
    );

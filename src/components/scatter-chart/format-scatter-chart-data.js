class FormatChartData {
  ...
  createDropdownList(d) {
    ...
    this.campusName = this.dropdownList.options[
      this.dropdownList.selectedIndex
    ].value;
    return this.getDataByCourse(
      this.groupedByCampus[this.campusName],
      this.campusName
    );
  }
  ...
  updateChartData(i) {
    document
      .querySelector(".dot-scatter")
      .classList.remove(`dot-${this.campusName.replace(" ", "")}`);
    ...
    document
      .querySelector("#campusName")
      .classList.add(`title-${this.campusName.replace(" ", "")}`);
    document
      .querySelector(".dot-scatter")
      .classList.add(`dot-${this.campusName.replace(" ", "")}`);
    const courseData = this.getDataByCourse(
      this.groupedByCampus[this.campusName],
      this.campusName
    );
    return courseData;
  ...
  getMaxCount(prop) {
    ...
    Object.keys(this.dataFormatted).forEach((i) => {
      maxofEachCourse.push(
        Math.max.apply(
          null,
          this.dataFormatted[i].map((obj) => obj[prop])
        )
      );
    });

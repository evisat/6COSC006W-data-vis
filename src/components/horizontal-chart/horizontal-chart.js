...
class CommuteAttendance {
  constructor(data) {
    ...
    this.container = document.querySelector(
      ".dv-horizontal-container-form--chart"
    );
    ...
    document.querySelector(
      ".dv-horizontal-container-form--description"
    ).innerHTML =
      "The scatterplot below shows a relationship between average commute length and average module marks.";
    document.querySelector(
      ".dv-horizontal-container-form--description-two"
    ).innerHTML =
      "Below are the top 5 course by average module mark: <i>(Click on a course)</i>";
    ...
    topFiveCourse.forEach((course) => {
      ...
      button.addEventListener("click", (e) => {
        ...
        const degreeOptions = Array.from(
          document.querySelectorAll("#select-degreeType-commute option")
        );
        ...
        const selectedDegree = degreeOptions.filter(
          (option) => option.value === this.dataFormatter.degree
        );
        ...
        const courseOptions = Array.from(
          document.querySelectorAll("#select-course-commute option")
        );
        courseOptions.forEach((option) => option.removeAttribute("selected"));

        // update course dropdown
        const selectedCourse = courseOptions.filter(
          (option) => option.value === this.dataFormatter.course
        );
        selectedCourse[0].setAttribute("selected", "");
        selectedCourse[0].innerHTML = this.dataFormatter.course;

        d3.select(
          `.dv-horizontal-container-form--chart .container--${this.course.replace(
            /\s|\&|\:|\(Sandwich\)/g,
            ""
          )}-commute`
        ).remove();
      ...
      document
        .querySelector(".dv-horizontal-container-form--buttons")
        .appendChild(button);
    });
  }
  ...
  addXAxis() {
    this.svg.append("g").attr("class", "x-axis-commute");
    this.formatXAxis();
  }
  ...
  formatXAxis() {
    d3.select(document.querySelector(".x-axis-commute"))
      ...
      .call(d3.axisBottom(this.x).tickSizeOuter(0).ticks(4));
  }
  ...
  addYAxis() {
    this.svg.append("g").attr("class", "y-axis-commute");
    this.formatYAxis();
  }

  formatYAxis() {
    d3.select(document.querySelector(".y-axis-commute")).call(
  ...
  addDots(course) {
    d3.select(
      `.container--${this.course.replace(/\s|\&|\:|\(Sandwich\)/g, "")}-commute`
    )
      .append("g")
      .selectAll(".dot")
      .data(course)
  ...
  addEventListeners() {
    document
      .querySelector("#select-degreeType-commute")
    ...
    document
      .querySelector("#select-course-commute")
      .addEventListener("change", (i) => {
  ...
  resize() {
    const chart = d3.select(
      document.querySelector(".dv-horizontal-container-form--chart svg")
    );
    ...
    chart
      .select(".x-axis-commute-label")
      .attr(
        "transform",
        `translate(${
          (this.width - this.margin.left - this.margin.right) / 2 - 50
        }, 220)`
      );

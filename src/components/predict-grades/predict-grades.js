...
class PredictGrades {
  constructor(data) {
    ...
    this.container = document.querySelector(
      ".dv-prediction-container-form--chart"
    );
    ...
    document.querySelector(
      ".dv-prediction-container-form--description"
    ).innerHTML =
  ...
  drawScatterplot() {
    ...
    this.svg
      ...
      .attr(
        "class",
        `container--${this.course.replace(/\s|\&|\:|\(Sandwich\)/g, "")}`
      );
  ...
  addXAxis() {
    this.svg.append("g").attr("class", "x-axis-predict");
    this.formatXAxis();
  }
  ...
  formatXAxis() {
    d3.select(document.querySelector(".x-axis-predict"))
      ...
      .call(d3.axisBottom(this.x).tickSizeOuter(0).ticks(5));
  }
  ...
  addYAxis() {
    this.svg.append("g").attr("class", "y-axis-predict");
    this.formatYAxis();
  }

  formatYAxis() {
    d3.select(document.querySelector(".y-axis-predict")).call(
      d3.axisLeft(this.y).ticks(4).tickSize([-this.width])
    );
  }
  ...
  addEventListeners() {
    document
      .querySelector("#select-degreeType")
      .addEventListener("change", (i) => {
        this.data = this.dataFormatter.updateCourseDropdown(i);
        d3.select(
          `.dv-prediction-container-form--chart .container--${this.course.replace(
            /\s|\&|\:|\(Sandwich\)/g,
            ""
          )}`
        ).remove();
  ...
  createRangeSlider() {
    ...
    this.rangeSlider.min = `${Math.min.apply(
      Math,
      this.data.map(function (o) {
        return o["perc_attendance"];
      })
    )}`;
    this.rangeSlider.max = `${Math.max.apply(
      Math,
      this.data.map(function (o) {
        return o["perc_attendance"];
      })
    ...
    document.querySelector(".prediction-slider-label").innerHTML =
      "Select attendance";
    const perctsliderTxt = document.querySelector("#perct-slider-value");

    predictedAverageTxt.innerHTML =
      "Average module mark: <span id='pred-color'>" +
      this.getPredictedMarks(this.rangeSlider.value) +
      "%</span>";
  ...
  updatePredictedMarks() {
    const chart = d3.select(
      document.querySelector(".dv-prediction-container-form--chart svg")
    );
    this.percValue = this.rangeSlider.value;
    this.pValue = (this.b0 + this.percValue * this.b1).toFixed(2);

    chart
      .select(`.linesOfIntersection-y`)
    ...
    chart
      .select(`.linesOfIntersection-x`)
      .attr("x1", this.x(0))
      .attr("x2", this.x(parseFloat(this.percValue)) - 7)
      .attr("y1", this.y(this.pValue))
      .attr("y2", this.y(this.pValue));

    document.querySelector("#predict-average-mark").innerHTML =
      "Average module mark: <span id='pred-color'>" + this.pValue + "%</span>";
    document.querySelector(
      "#perct-slider-value"
    ).innerHTML = `${this.percValue}%`;
  }
  ...
  resize() {
    ...
    chart
      .select(".x-axis-predict-label")
      .attr(
        "transform",
        `translate(${
          (this.width - this.margin.left - this.margin.right - 10) / 2
        }, 225)`
      );

    // update dots
    chart
      .selectAll(`.dot-${this.data[0]["course_code"]}`)
      .attr("cx", (d) => this.x(d["perc_attendance"]))
      .attr("cy", (d) => this.y(d["average_modulemark"]));
    ...
    const line = d3
      ...
      .y((d) => this.y(d.yhat));
    chart
      .select(`.predict-trend-line-${this.data[0]["course_code"]}`)
      .attr("d", line);
